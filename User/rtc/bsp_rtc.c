...
/*星期，生肖用文字ASCII码*/
//uint8_t const *WEEK_STR[29] = {"日", "一", "二", "三", "四", "五", "六"};
//uint8_t const *zodiac_sign[] = {"猪", "鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗"};
extern  char  Usart1_recev_buff[300];
extern  uint16_t  Usart1_recev_count;             //USART1发送计数器
extern  char  PowerOffReset ;                 //掉电重启标志位
...
//		Time_Adjust(tm);                          //禁用串口输入功能，防止程序一直处于等待输入状态
...
void RTC_Configuration(void)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR | RCC_APB1Periph_BKP, ENABLE);
	/*Enable WKUP pin*/
//	PWR_WakeUpPinCmd(ENABLE);         //是否开启有待确认
...
//	RTC_ITConfig(RTC_IT_SEC, ENABLE);         //暂时关闭RTC秒中断
...
//static uint8_t USART_Scanf(uint32_t value)    //本函数未用到，后续应该删除
//{
//	  uint32_t index = 0;
//	  uint8_t tmp[2] = {0, 0};
//	
...
//	    if ((tmp[index] <'0') || (tmp[index] > '9'))   /*数字0到9的ASCII码为0x30至0x39*/
//	    { 		  
...
//	  Time_Regulate(tm);
	  {	
			tm.tm_year =TimeCorrect->Time_Year+2000;  //对输入的年份进行修正
	  ...
	  RTC_WaitForLastTask();
	 	...
	 	BKP_WriteBackupRegister(BKP_DR1, 0xA5A5);     //更新备份寄存器的值
    RTC_WaitForLastTask();
...
//		RTC_WaitForLastTask();
//		ParaUpdateCheck();                            //将配置参数再次写入BKP存储区，因为每次更新RTC寄存器时都将把BKP存储区清空，需要再次写入
...
//	  }	 	  	
	  ...
	  /* 输出时间戳，公历时间 */
//	  printf(" \r\nUNIX时间戳 = %d 当前时间为: %d年(%s年) %d月 %d日 (星期%s)  %0.2d:%0.2d:%0.2d\r\n",TimeVar,
