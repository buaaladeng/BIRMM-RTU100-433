...
struct liquid_set 
{
	uint16_t  CollectPeriod;         //采集间隔             默认15分钟采集一次
	uint16_t  SendCount;            //每次上报的上报周期    默认一小时上报一次
  ...
  u8        BatteryCapacity;        //电池容量检测，例如xx%，仅为百分号前面的部分。
	...
	u8    MessageSetFlag;         //用于指示当前是否修改参数配置
...
typedef enum 
{
  ...
  DEVICE_WAKEUP= 0x60000200,    //设备状态指示（唤醒）
	...
	LOW_LEVEL    = 0x10000043,
	HIGH_LEVEL   = 0x10000044,
	SET_REQUEST  = 0x10000200      //主动请求下发配置
...
void DeviceSetRequest(u8* pSendBuff, u8* pDeviceID, u16 NodeAddress);                   //主动请求
