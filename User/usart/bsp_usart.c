...
void USART2_Config(void)
{
	...
	/* 使能串口2接收中断 */
	//USART_ClearITPendingBit(USART2,USART_IT_TC);            //清除接收数据完成等待中断标志位
 //  USART_GetFlagStatus(USART2,USART_FLAG_TC);
 // USART_ClearFlag(USART2,USART_FLAG_TC);                  //清除接收数据完成中断标志位
  USART_ITConfig(USART2, USART_IT_IDLE, ENABLE);          //使能串口空线中断
...
void USART3_Config(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	/* Configure SIM5216 POWER_ON (PC.07) as output push-pull */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
...
void UART_NVIC_Configuration(void)
{
 ...
 /* Enable the USART2 Interrupt */
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;             //串口2与串口3每次只能用一个，因此可配置为相同的
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
...
//		DMA_InitStructure.DMA_Priority = DMA_Priority_High;       //DMA_Priority_High
		...
		/*配置DMA1的3通道*/		   
		DMA_Init(DMA1_Channel6, &DMA_InitStructure); 	              //USART3接收通道为：DMA1_Channel3
		/*使能DMA*/
